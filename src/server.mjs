...
export function createBridge({ port = 4328, token = randomBytes(24).toString('hex') } = {}) {
  let active = 0;
  const server = http.createServer(async (req, res) => {
    ...
    if (req.headers.host !== `127.0.0.1:${boundPort}`) return reply(403, { error: 'Invalid host' });
    ...
    if (['/api/session', '/api/fixture-session'].includes(url.pathname) && req.method === 'GET') {
      if (
        extension ||
      ...
      if (url.pathname === '/api/fixture-session') {
        const sample = JSON.parse(
          await readFile(path.join(root, 'examples/common-tools.json'), 'utf8'),
        );
        const catalog = validateCatalog(sample.tools).map(({ enabled, ...tool }) => tool);
        return reply(200, { token, catalog, fixture: true });
      }
    ...
    const paths = {
      ...
      '/catalog.js': 'extension/catalog.js',
      '/extension/catalog.js': 'extension/catalog.js',
      '/style.css': 'public/style.css',
      '/fixture': 'public/fixture.html',
      '/fixture.js': 'public/fixture.js',
      '/adapter.js': 'extension/content.js',
      '/icon.svg': 'public/icon.svg',
      '/examples/common-tools.json': 'examples/common-tools.json',
      '/examples/pantry-demo.json': 'examples/pantry-demo.json',
    };
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  server.on('error', (error) => {
    console.error(`Unable to start jev2mcp: ${error.message}`);
    process.exitCode = 1;
  });
  server.on('listening', async () => {
    try {
      ...
      console.error(`Unable to save the jev2mcp pairing token: ${error.message}`);
      server.close();
      process.exitCode = 1;
    }
  });
